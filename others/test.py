...
def get_args():
    ...
    parser.add_argument('--resume_from', type=str, default='/home/zhangq/Desktop/zhangq/LidarBEVTrack/work_dir/20220930_15:26:02', help='resume from dir')
    # '/home/zhangq/Desktop/zhangq/LidarBEVTrack/work_dir/20220930_15:26:02'
...
def test(args):
    ...
    all_pathVal = DataPathTrain.sheet_by_index(0).col_values(0)
    ...
    if args.resume_from is not None:
        # training resume
        checkpoint = torch.load(os.path.join(exp_checkpoint_dir, 'epoch70.pth'))
                ...
                with torch.no_grad():
                    for step, (lidar_bev, One_Stage_HM, LW_label, Alpha_label) in tqdm(enumerate(dataloader_val), total=len(dataloader_val), smoothing=0.9):
                        ...
                        if step == 0:
                            ...
                            top_k_indices, peak_heat_map = HeatMapNMS(center_hm, kernel_size=3, max_num_objects=10, score_threshold=0.5)
                            ...
                            save_name = '/home/zhangq/Desktop/zhangq/LidarBEVTrack/results/' + str(step) + '_val.jpg'
...
def HeatMapNMS(heat_map_scores, kernel_size, max_num_objects, score_threshold):
    ...
    batch_index = torch.arange(1, dtype=torch.float, device="cuda:0") * grid_height * grid_width * 1

    flattened_indices = (
            torch.reshape(batch_index, [bs, 1, 1]) + batch_top_k_indices * 1 +
            torch.reshape(torch.arange(1, dtype=torch.float, device="cuda:0"), [1, 1, 1]))
    flattened_indices = torch.reshape(flattened_indices, [-1]).type(torch.int)
    top_k_indices = torch.as_tensor(np.array(np.unravel_index(flattened_indices.cpu(), (grid_height, grid_height))))
    ...
    min_num = torch.tensor(1e-4, device="cuda:0")
    filtered_heat_map.data = torch.where(filtered_heat_map.data >= score_threshold, filtered_heat_map.data, min_num)

    # print('top_k_indices:\n', top_k_indices)
    # print('peak_heat_map:\n', filtered_heat_map)

    return top_k_indices, filtered_heat_map
