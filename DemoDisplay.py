...
def run():
    ...
    all_img_detections = np.load('./TrackResults/all_img_detections.npy')  # img proj dets, (x,y, frame_num, group_num) 4
    all_detection2Dboxes = np.load('./TrackResults/all_detection2Dboxes.npy')   # img 2D bbox (tlX, tlY, w, h, depth, frame_num, group_num)
    ...
    for num_group in range(len_groups+1):
        ...
        if plot3:
            ...
            for i in range(maxLength):
                ...
                plt.xlim(-20, 20)
                plt.ylim(0, 40)
                plt.scatter(radar_detectionsCurr[:, 0], radar_detectionsCurr[:, 1], color='b', marker='s', s=5,
                            alpha=0.5)
                ...
                # if len(estimated_tracksCurr) > 0:
                #     plt.plot(estimated_tracksCurr[:, 1], estimated_tracksCurr[:, 2], color='r', alpha=1)
                # if len(gt_tracksCurr) > 0:
                #     plt.plot(gt_tracksCurr[:, 1], gt_tracksCurr[:, 2], color='k', linestyle='--', alpha=0.8)
                #
                # plt.show()
                # plt.clf()
                # print()
        ...
        # result_name = group_name[0] + '_' + group_name[1] + '.png'
