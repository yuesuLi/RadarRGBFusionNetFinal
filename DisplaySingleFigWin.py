...
def run():
    ...
    for num_group in range(len_groups+1):
        ...
        plt.xlim(-7, 7)
        plt.ylim(0, 50)
        plt.xlabel('X / m', fontdict={'fontsize': 15, 'fontweight': 'bold', 'fontname': 'Arial'})
        plt.ylabel('Z / m', fontdict={'fontsize': 15, 'fontweight': 'bold', 'fontname': 'Arial'})
        plt.tick_params(labelsize=15)
        title_name = str(int(group_name[0])) + '_' + group_name[1]
        # plt.title('Traditional Fusion Algorithm') # Proposed Algorithm, Traditional Fusion Algorithm, Camera Only, 单相机追踪, 传统融合追踪算法, 提出的融合追踪算法
        ...
        if plot2:
            ...
            for i in ids:
                ...
                #     color = 'g'
                # ii = ii + 1
                if id_num < est_ids_length:
                    plt.plot(estimated_tracks_mask[:, 1], estimated_tracks_mask[:, 2], color='r', alpha=1)
                else:
                    # 单相机跟踪算法，传统融合跟踪算法，引入外观和方向相似度，引入分级匹配方法，本文提出的算法，____引入外观和方向度量
                    plt.plot(estimated_tracks_mask[:, 1], estimated_tracks_mask[:, 2], color='r', alpha=1,
                             label='本研究提出的算法')
            ...
            for j in ids:
                ...
                if gt_id_num < gt_ids_length:
                    plt.plot(gt_tracks_mask[:, 1], gt_tracks_mask[:, 2], color='k', linestyle='--', alpha=1)
                else:
                    plt.plot(gt_tracks_mask[:, 1], gt_tracks_mask[:, 2], color='k', linestyle='--', alpha=1,
                             label='真实轨迹')
                # gt_line, = plt.plot(gt_tracks_mask[:, 1], gt_tracks_mask[:, 2], color='k', linestyle='--', alpha=1)

        legend = plt.legend(loc='lower left', fontsize=15)
        # legend.texts[0].set_weight('bold')
        result_name = str(int(group_name[0])) + '_' + group_name[1] + '.png'
        SaveRawPath = r'D:\ZJU\嗑盐\代码备份\RadarRGBFusionNet2_20240402\TrackResults\Figs'
