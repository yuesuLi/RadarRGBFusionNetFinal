...
def draw_detections_in_BEV(detections_r, detections_c, lidar_json, OCU_json, rgb_json, group_num):
    """
    ...
    """
    Lidar_annotation_points = OCUpts2rbev(lidar_json, OCU_json, rgb_json)
...
def get_detections_and_labels(detections_r, detections_c, lidar_json, OCU_json, rgb_json, Lidar_points, frame_num):
    """
    ...
    if 'annotation' in lidar_json:
        Lidar_annotation_points = lidar2img_filter(lidar_json, OCU_json, rgb_json, image_width, image_height, Lidar_points)
    else:
        Lidar_annotation_points = np.zeros(shape=(0, 7))
    # Lidar_annotation_points2 = OCUpts2rbev(lidar_json, OCU_json, rgb_json)
    radar_center = np.array([detections_r[i].center for i in range(len(detections_r))])
    cam_proj_center = np.array([detections_c[i].center for i in range(len(detections_c))])
    Camera2Dboxes = np.array([np.append(detections_c[i].tlwh.numpy(), detections_c[i].center[1]) for i in range(len(detections_c))])
    ...
    if len_img != 0:
        ...
        img2DBoxes = np.hstack((Camera2Dboxes, np.ones((len_img, 1)) * frame_num, np.ones((len_img, 1)) * -1))
    else:
        img_dets = np.zeros(shape=(0, 4))
        img2DBoxes = np.zeros(shape=(0, 7))

    return Lidar_annotations, radar_dets, img_dets, img2DBoxes
...
def run(opt):
    ...
    all_detection2Dboxes = np.zeros(shape=(0, 7))   # (tlX, tlY, w, h, depth, frame_num, group_num)
    ...
    for i in range(len(datasets_path)):
        ...
        dataset = self_dataset3(source, img_size=imgsz)
        group_num += 1
        ...
        detection2Dboxes = np.zeros(shape=(0, 7))  # (tlX, tlY, w, h, depth, frame_num, group_num)

        for frame_idx, (rgb_img0, img, rgb_json, OCU_data, OCU_json, lidar_json, Lidar_points) in enumerate(dataset):
            # print('frame_idx:', frame_idx)

            # if frame_idx == 138:
            #     print()
            outputs, detections_c, im0 = detection_model.get_detections(rgb_img0, img, rgb_json, OCU_json, OCU_data)
            # detectionsGT_c = detection_model.get_GT_detections(rgb_img0, rgb_json, OCU_json)
            ...
            Lidar_annotations, radar_dets, img_dets, img2DBoxes = get_detections_and_labels(detections_r, detections_c, lidar_json,
                                                                                OCU_json, rgb_json, Lidar_points, frame_idx)
            ...
            detection2Dboxes = np.vstack((detection2Dboxes, img2DBoxes))
        ...
        all_detection2Dboxes = np.vstack((all_detection2Dboxes, detection2Dboxes))

        print('group:', group_num, ', done.')
        # np.savez('./TrackResults/' + datasets_path[i] + '.npz', estimated_tracks=estimated_tracks, gt_tracks=gt_tracks,
        #          radar_detections=radar_detections, img_detections=img_detections, detection2Dboxes=detection2Dboxes)
    ...
    np.save("./TrackResults/all_detection2Dboxes.npy", all_detection2Dboxes)
