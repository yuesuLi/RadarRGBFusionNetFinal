...
def draw_radarRawPoints_in_BEV(OCU_data):
    ...
    plt.show()
    plt.pause(0.1)
    # save_name = '/mnt/ChillDisk/personal_data/zhangq/RadarRGBFusionNet2_20231128/results/radarRawPoints.png'
...
def draw_radarCulPoints_in_BEV(RadarDetections):
    ...
    plt.show()
    plt.pause(0.1)
    # save_name = '/mnt/ChillDisk/personal_data/zhangq/RadarRGBFusionNet2_20231128/results/radarCulPoints.png'
    # plt.savefig(save_name, dpi=500)
    plt.clf()
...
def draw_detections_in_BEV(detections_r, detections_c, lidar_json, OCU_json, rgb_json, curr_label, group_num):
    ...
    plt.show()
    plt.pause(0.1)
    # save_name = '/mnt/ChillDisk/personal_data/zhangq/RadarRGBFusionNet2_20231128/results/AllDetBEV.png'
    # plt.savefig(save_name, dpi=500)
    plt.clf()
...
def draw_Split_detections_in_BEV(fusion_detections, camera_detections, radar_detections, lidar_json, OCU_json, rgb_json, curr_label, group_num):
    ...
    plt.show()
    plt.pause(0.1)
    # save_name = '/mnt/ChillDisk/personal_data/zhangq/RadarRGBFusionNet2_20231128/results/SplitDetBEV.png'
    # plt.savefig(save_name, dpi=500)
    plt.clf()
...
def get_fusion_detections(detections_f, frame_num):

    fusion_center = np.array([detections_f[i].center for i in range(len(detections_f))])
    len_fusion = fusion_center.shape[0]

    if len_fusion != 0:
        fusion_dets = np.hstack((fusion_center, np.ones((len_fusion, 1)) * frame_num, np.ones((len_fusion, 1)) * -1))
    else:
        fusion_dets = np.zeros(shape=(0, 4))
...
def run(opt):
    ...
    # Dataloader
    file_path = '/mnt/ChillDisk/personal_data/zhangq/RadarRGBFusionNet2_20231128/GroupPath/20240427.xlsx'  # 20240427, 20231207
    ...
    all_fusion_detections = np.zeros(shape=(0, 4))  # save img dets, (x,y, frame_num, group_num)
    all_gt_detections = np.zeros(shape=(0, 7))  # save gts, (id,x,y,z, classes, frame_num, group_num)
    ...
    for group_num, groupname in enumerate(datasets_path):
        ...
        fusion_detections = np.zeros(shape=(0, 4))  # save fusion dets, (x,y, frame_num, group_num)
        gt_detections = np.zeros(shape=(0, 7))  # save gts, (id,x,y,z, classes, frame_num, group_num)
        ...
        for frame_idx, (rgb_img0, img, rgb_json, OCU_data, OCU_json, lidar_json, extendGT) in enumerate(dataset):
            ...
            OCU_points, OCU_db, OCU_num_dbscan = get_dbscan_points(OCU_data, 1, 4)
            ...
            fusion_dets = get_fusion_detections(Fdetections, frame_idx)
            ...
            fusion_detections = np.vstack((fusion_detections, fusion_dets))
            gt_detections = np.vstack((gt_detections, Lidar_annotations))
            # print()

        radar_detections[:, 3] = group_num
        img_detections[:, 3] = group_num
        fusion_detections[:, 3] = group_num
        gt_detections[:, 6] = group_num
        print('img_detections_num:', len(img_detections))

        all_fusion_detections = np.vstack((all_fusion_detections, fusion_detections))
        all_gt_detections = np.vstack((all_gt_detections, gt_detections))
    ...
    np.save("./TrackResults/all_fusion_detections.npy", all_fusion_detections)
    np.save("./TrackResults/all_gt_detections.npy", all_gt_detections)
    np.save("./TrackResults/all_radar_detections_RMSE.npy", all_radar_detections)
    np.save("./TrackResults/all_img_detections_RMSE.npy", all_img_detections)
